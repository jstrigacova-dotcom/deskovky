...
SELECT * FROM bgg_dataset_des WHERE `Year Published` IS NULL;
    #chci filtrovat řádek ve sloupci roku vydání, jelikož vyšel v úvodu jeden řádek, ale spuštěním příkazu se nic nedohledá, tak se pravděpodobně jednalo o záznam, kde chybělo i ID z předchozího dotazu
...
AND `Max Players` IS NOT NULL;
    ...
    #Tak tady fakt vím nic. Dle netu například hry Stonewall Jackson, Bitter Woods, Wagram jsou výhradně pro 2 hráče. Hra Somosierra 1808 má max players 1 a min by mělo být také 1. Narnia a China 2 - 4 hráči, tak mám nastavit 2? Čili 
    #UPDATE `bgg_dataset_des` SET `Min Players` = 1 WHERE `ID` = 39279; pro Somosierru 
...
SELECT * FROM bgg_dataset_des WHERE `Owned users ` IS NULL;
    #filtruju si sloupec owned users a zjišťuju, že záznamů je už jen 7. Ostatní sloupce mají řádky v pořádku vyplněny a tak se nedokážu rozhodnout, jestli pro budoucí analýzu, tam ponechám null hodnoty a nebo je dám na 0 jako že nevlastní nikdo
    #UPDATE bgg_dataset_des SET `Owned Users` = 0 WHERE `Owned Users` IS NULL 
...
WHERE Domains = &#39;&#39; OR TRIM(Domains) = &#39;&#39;;
    #tady teda zjišťuju, že prázdný text u sloupce domains mám u 10 140 záznamů a u 3 teda mám NULL. Tak teď úplně nevím jestli dát
    #UPDATE `bgg_dataset_des`
      SET `Domains` = NULL
      WHERE `Domains` = &#39;&#39; OR TRIM(`Domains`) = &#39;&#39; a převést na NULL
    #UPDATE `bgg_dataset_des`
      SET `Domains` = &#39;&#39;
      WHERE `Domains` IS NULL; a NULL převést na prázdno
...
ORDER BY `Play Time` DESC;
    #tady se dívám na zoubek hrám, co se hrají i více než 100 hodin a výsledek mi vychází takovýto. Provádím si tedy research na webu a zjišťuju, že opravdu hra/hry, které se hrají i tisíc hodin existují, whaaat? Ok, takže data jsou v pořádku, ale zase ty nuly …. nemilosrdně je měním na NULL
